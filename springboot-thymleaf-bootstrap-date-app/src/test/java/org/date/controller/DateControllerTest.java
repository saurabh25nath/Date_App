...
import static org.date.matchers.GlobalErrorsMatcher.hasGlobalError;
...
import org.date.controller.DateController;
...
@WebMvcTest(controllers = DateController.class, includeFilters = @Filter(classes = Service.class, type = FilterType.ANNOTATION))
public class DateControllerTest {
	...
	@Test
	public void testNoOfDaysSuccess() throws Exception {
		mockMvc.perform(
				MockMvcRequestBuilders.post("/noofdays").accept(MediaType.TEXT_HTML)
				.param("startDate", "1/01/2019")
				.param("endDate", "31/01/2019")
		)
		.andExpect(status().isOk())
		.andExpect(view().name("noofdays"))
		.andExpect(model().hasNoErrors())
		.andExpect(model().attribute("noOfDays", equalTo(30L)))
	...
	public void testErrorWhenNoDates() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.post("/noofdays").accept(MediaType.TEXT_HTML))
		.andExpect(model().attributeHasFieldErrorCode("dates", "startDate", "NotNull"))
		.andExpect(model().attributeHasFieldErrorCode("dates", "endDate", "NotNull"))
